    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gro_savebtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                gro_date.updateDate(gro_date.getYear(),gro_date.getMonth(),gro_date.getDayOfMonth());//아무것도안누르고 입력버튼 누를시 현재 날짜거 가져옴 지우면안됨
                int gro_check;
                FirebaseDatabase database = FirebaseDatabase.getInstance();
                DatabaseReference uploadgrowthresult = database.getReference("growthresult");
                final Integer age = agecheck(gro_date.getYear(), gro_date.getMonth(), gro_date.getDayOfMonth(), year1, month1, day1);//기록 날자로 생후 일수 계산
                ...
                getheight.yValue = height;
                getweight.yValue = weight;
                getheight.xValue = age;
                getweight.xValue = age;
                        ...
                        getgro.addValueEventListener(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                ...
                                for (DataSnapshot message : dataSnapshot.child("heightresult").getChildren()) {
                                    ...
                                    if (heit != null) {
                                        if (heit.getxValue() == age) {
                                            for (DataSnapshot message2 : dataSnapshot.child("weightresult").getChildren()) {
                                                weit = message2.getValue(weightage.class);
                                                if (weit != null) {
                                                    if (weit.getxValue() == age) {//생후 일자로 계산해서 그날의 데이터값 가져옴
                                                        dataview.setText(gro_date.getYear() + "년 " + (gro_date.getMonth() + 1) + "월 " + gro_date.getDayOfMonth() + "일 " + " 신장:" + heit.getyValue() + "cm 몸무게:" + weit.getyValue() + "kg");//날짜 선택시 아래 텍스트로 해당날짜 데이터 출력
                                                        break;
                                                    } else {
